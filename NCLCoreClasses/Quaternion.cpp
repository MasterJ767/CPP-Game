...
Quaternion Quaternion::LookAt(Vector3 position, Vector3 destination, Vector3 forward) {
	Vector3 newForward = (destination - position).Normalised();

	float dot = Vector3::Dot(forward, newForward);
	...
	Vector3 rotAxis = Vector3::Cross(forward, newForward).Normalised();
	return Quaternion::AxisAngleToQuaterion(rotAxis, rotAngle);
}
