...
void PhysicsSystem::ImpulseResolveCollision(GameObject& a, GameObject& b, CollisionDetection::ContactPoint& p) const {
	if (a.GetBoundingVolume()->isTrigger || b.GetBoundingVolume()->isTrigger) { return; }
	...
	if (a.GetCollisionLayer() == HUMAN || b.GetCollisionLayer() == HUMAN) {
		cRestitution = 0.75f;
	}
...
void PhysicsSystem::BroadPhase() {
	...
	QuadTree<GameObject*> tree(Vector2(1024, 1024), 8, 6);
...
void PhysicsSystem::NarrowPhase() {
	for (std::set<CollisionDetection::CollisionInfo>::iterator i = broadphaseCollisions.begin(); i != broadphaseCollisions.end(); ++i) {
		...
		if (CollisionDetection::ObjectIntersection(info.a, info.b, info)) {
			...
			ImpulseResolveCollision(*info.a, *info.b, info.point);
			allCollisions.insert(info);
			//Debug::DrawLine(info.a->GetTransform().GetPosition(), info.b->GetTransform().GetPosition(), Vector4(0.5f, 1, 0, 1), 4);
		}
	}
}
