...
const char DOOR_1_NODE_X = 'd';
const char DOOR_1_NODE_Z = 'h';
const char DOOR_2_NODE_X = 'p';
const char DOOR_2_NODE_Z = 'q';
const char FLOOR_NODE	 = '.';
const char BRIDGE_NODE   = 'b';
const char VOID_NODE     = 'v';
const char BUTTON_NODE   = 'o';
...
NavigationGrid::NavigationGrid(const std::string&filename, const Vector3 offset) : NavigationGrid() {
	...
	for (int y = 0; y < gridHeight; ++y) {
		for (int x = 0; x < gridWidth; ++x) {
			...
			infile >> type;
			n.type = type;
			n.position = Vector3((float)(x * nodeSize) + offset.x, offset.y, (float)(y * nodeSize) + offset.z);
		}
	}
	...
	for (int y = 0; y < gridHeight; ++y) {
		for (int x = 0; x < gridWidth; ++x) {
			...
			for (int i = 0; i < 4; ++i) {
				if (n.connected[i]) {
					if (n.connected[i]->type == FLOOR_NODE || n.connected[i]->type == BUTTON_NODE) {
						n.costs[i]		= 1;
					}
					else if ((n.connected[i]->type == DOOR_1_NODE_X || n.connected[i]->type == DOOR_1_NODE_Z) && !door1Active) {
						n.costs[i]      = 1;
					}
					else if ((n.connected[i]->type == DOOR_2_NODE_X || n.connected[i]->type == DOOR_2_NODE_Z) && door1Active) {
						n.costs[i]      = 1;
					}
					else {
...
GridNode* NavigationGrid::GetNodes() {
	return allNodes;
}

void NavigationGrid::SwitchDoors() {
