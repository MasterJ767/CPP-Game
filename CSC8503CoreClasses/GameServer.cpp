...
void GameServer::SendPacket(GamePacket& packet, ENetPeer* peer) {
	ENetPacket* dataPacket = enet_packet_create(&packet, packet.GetTotalSize(), 0);
...
void GameServer::UpdateServer() {
	...
	while (enet_host_service(netHandle, &event, 0) > 0) {
		...
		if (type == ENetEventType::ENET_EVENT_TYPE_CONNECT) {
			...
			clients.emplace(clientMaxID, p);
			ClientConnectPacket* packet = new ClientConnectPacket(clientMaxID);
			SendGlobalPacket((GamePacket&)packet);

			std::cout << "Server: Assigned client ID " << clientMaxID << std::endl;
